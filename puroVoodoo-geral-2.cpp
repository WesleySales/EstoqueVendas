...
void listarProdutos(produto produtos[], int contador_produto){
	...
	for(int i=0;i<contador_produto;i++){
		...
		printf("\nproduto %d: %s\n",i+1, produtos[i].nome);
		printf("preco: R$ %.2lf\n",produtos[i].preco);
        printf("quantidade disponivel: %d UN\n",produtos[i].estoque);
...
void adicionarAoCarrinho(ItemCarrinho carrinho[],produto produtos[], int *numItens, int produtoId, int quantidade) {
    ...
    if (itemExistente != -1) {
        ...
        printf("Limite de estoque ultrapasado");
    ...
    for (int i = 0; i < numItens; i++) {
        ...
        for (int j = 0; j < numProdutos; j++) {
            if (produtos[j].id == produtoId) {
                ...
                printf("ID %d  -  Produto: %s - Qtd: %d  -  Pre?o Und: %.2lf  -  Pre?o Total: %.2lf\n",
                   carrinho[j].produtoId,produtos[j].nome, quantidade, produtos[j].preco, precoTotal);
                   break;
            }
        }
    }
...
void deletarProduto(produto produtos[], int busca_id,int contador_produto){
	  // Procurar pelo SKU e excluir
                    for (int i = 0; i <= contador_produto; i++) {
                        if (produtos[i].id == busca_id) {
                            ...
                            produtos[i] = produtos[contador_produto - 1];
...
void deletarItemCarrinho(ItemCarrinho carrinho[],produto produtos[], int busca_id, int numItens){
	  // Procurar pelo SKU e excluir
                    
                        //carrinho[busca_id-1].produtoId
                            printf("\nProduto excluído:\n");
                            // Movendo o último produto para a posição atual e decrementando o contador
                            carrinho[busca_id-1] = carrinho[numItens - 1];
                            
                            printf("\nProduto excluído do carrinho com sucesso.\n\n");

                            valor_total -= produtos[busca_id-1].preco * carrinho[busca_id-1].quantidade;
                            totalComDesconto = calcularDesconto(valor_total);
...
void editarProduto(int busca_id, produto produtos[],int numItens){
	...
	int indice = busca_id-1;
 	...
 	//	if (busca_id == produtos[j].id) {
			...
			fflush(stdin);
		//	produtos[j].estoque=edit_estoque;
      //  }
 	//}
}
...
//	int contador_carrinho=0;
	...
	do{
		...
		printf("\nExcluir Produto\n");  
                ...
                scanf("%d", &busca_id);
                fflush(stdin);      
				deletarProduto( produtos,  busca_id, contador_produto);  
				contador_produto--;       
		break;
		...
		do{
			...
			switch(opcao){
		        ...
		        case 2:
	                ...
	                scanf("%d", &busca_id);
	                fflush(stdin);   
					deletarItemCarrinho(carrinho, produtos, busca_id, numItens);
                    numItens--;
			...
			case 5:
					...
					printf("Escolha o produto que deseja editar: ");
					scanf("%d",&busca_id);
					editarProduto( busca_id,  produtos, numItens);
					break;	
			case 6:
